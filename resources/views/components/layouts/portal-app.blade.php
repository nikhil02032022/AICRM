...
<head>
    ...
    <style>
        ...
        .portal-nav-active    { background-color: color-mix(in srgb, var(--portal-primary) 75%, #000); }
...
<body class="h-full font-sans antialiased" x-data="{ sidebarOpen: false }">
    ...
    >
        <div class="flex h-16 shrink-0 items-center justify-between px-4">
            ...
            @else
                <span class="text-white font-bold text-base truncate">{{ $branding['name'] }}</span>
            @endif
            <button type="button" @click="sidebarOpen = false" class="text-white hover:text-gray-200 ml-2">
    ...
    <div class="lg:pl-64 flex flex-col min-h-screen">
        ...
        {{-- Top bar --}}
        <div class="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-4
                    border-b border-gray-200 bg-white px-4 shadow-sm sm:px-6">
            ...
            @if (Route::has('portal.auth.logout'))
                <form method="POST" action="{{ route('portal.auth.logout') }}">
                    @csrf
                    <button type="submit"
        ...
        {{-- Flash messages --}}
        <div class="px-4 pt-4 sm:px-6 lg:px-8 space-y-2">
            @if (session('success'))
        ...
        <footer class="border-t border-gray-100 py-4 px-4 sm:px-6 lg:px-8">
