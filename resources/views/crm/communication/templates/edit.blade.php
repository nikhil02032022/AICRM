<x-layouts.crm title="Edit Communication Template">
    <div class="max-w-3xl space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 leading-tight">Edit Template</h1>
                <p class="mt-0.5 text-sm text-gray-500">Updating: <span class="font-medium text-gray-700">{{ $template->name }}</span></p>
            </div>
        </div>
        ...
        <form
            method="POST"
            action="{{ route('crm.communication.templates.update', $template->uuid) }}"
            x-data="{ channel: '{{ old('channel', $template->channel->value) }}', type: '{{ old('type', $template->type->value) }}' }"
        >
            @csrf
            @method('PATCH')
                            ...
                            <div class="relative mt-1">
                                ...
                                >
                                    @foreach (\App\Enums\CRM\CommunicationChannel::cases() as $c)
                                        <option value="{{ $c->value }}" @selected(old('channel', $template->channel->value) === $c->value)>
                                            {{ $c->label() }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="relative mt-1">
                                ...
                                >
                                    @foreach (\App\Enums\CRM\TemplateType::cases() as $t)
                                        <option value="{{ $t->value }}" @selected(old('type', $template->type->value) === $t->value)>
                                            {{ $t->label() }}
                                        </option>
                                    @endforeach
                                </select>
