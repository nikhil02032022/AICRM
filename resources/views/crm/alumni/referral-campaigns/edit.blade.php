{{-- BRD: CRM-AL-002 — Edit an existing alumni referral campaign --}}
<x-layouts.crm title="Edit Campaign — {{ $campaign->name }}">
    <x-slot:header>
    ...
    <form method="POST" action="{{ route('crm.alumni.referral.campaigns.update', $campaign) }}" class="max-w-2xl space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $campaign->description) }}</textarea>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $campaign->start_date->toDateString()) }}" required
                ...
                <div>
                    ...
                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $campaign->end_date?->toDateString()) }}"
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <select id="reward_type" name="reward_type" required
                        ...
                        <option value="{{ $type->value }}" {{ old('reward_type', $campaign->reward_type->value) === $type->value ? 'selected' : '' }}>
                            {{ $type->label() }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="reward_value" name="reward_value" value="{{ old('reward_value', $campaign->reward_value) }}" min="0" step="0.01"
